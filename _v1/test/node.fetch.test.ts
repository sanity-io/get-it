import {environment, getIt, type Middleware} from 'get-it'
...
/**
 * We're asserting that two requests responses are equal, but the date header might
 * differ by a second, so we remove it from the response.
 */
function withoutDateHeader(): Middleware {
  return {
    onResponse: (response) => {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const {date, ...rest} = response.headers
...
describe.runIf(typeof fetch !== 'undefined' && environment === 'node')(
  ...
  () => {
    ...
    it.skipIf(process.versions.node.split('.')[0] >= '20')('can turn off keep-alive', async () => {
      const request = getIt([baseUrl, promise(), withoutDateHeader()], nodeRequest)
      const expected = await request({url: '/plain-text', fetch: false})
    ...
    it('should be able to post a Buffer as body', async () => {
      const request = getIt([baseUrl, withoutDateHeader()], nodeRequest)
    ...
    it('should be able to use JSON request middleware', async () => {
      const request = getIt([baseUrl, jsonRequest(), withoutDateHeader()], nodeRequest)
      const req = request({url: '/echo', fetch: true, body: {foo: 'bar'}})
      await expectRequestBody(req).resolves.toEqual('{"foo":"bar"}')
    })

    it('should be able to set http headers', async () => {
      const request = getIt([baseUrl, jsonResponse(), withoutDateHeader()], nodeRequest)
