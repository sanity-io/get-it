...
# tsdown code-splits shared code into separate chunks, so dist/index.js is just
# a re-export shim. Bundle it back into a single file first so the size we report
# reflects what a real consumer's bundler would actually ship. We reuse rolldown
# (tsdown's bundler) so this needs no extra dependency.
