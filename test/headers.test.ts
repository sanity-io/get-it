...
describe('headers', () => {
  ...
  it.skipIf(adapter === 'fetch')('should set Content-Length based on body (Buffer)', async () => {
    const request = getIt([baseUrl, jsonResponse()])
  ...
  it.skipIf(adapter === 'fetch')('should set Content-Length based on body (string)', async () => {
    const request = getIt([baseUrl, jsonResponse()])
